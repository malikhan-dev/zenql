...
func TestSifuTrueFalseAnd(t *testing.T) {
	...
	).Take(1).Update(expr.Prop("Name").AppendString(" Updated").Gen()).Collect()
	...
	).Take(20).Update(expr.Prop("Name").AppendString(" Updated").Gen()).Collect()
...
func BenchmarkQueryEngineWithSifu(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		result := From(&items).Where(expr.Prop("Name").EqStr("Jane").And(expr.Prop("Flag").True()).Gen()).Collect()

		result2 := From(&result).Any(expr.Prop("Name").NotEqStr("Jane").Or(expr.Prop("Flag").False()).Gen()).Assert()
...
func TestGroupByNewWithSifu(t *testing.T) {
	...
	res :=
		...
		Group[bool, ComplexObjectToSearch](
			From(&items).Where(expr.Prop("Age").BInt(20).Gen()),
...
func TestValidFilterWithSifu(t *testing.T) {
	...
	GroupResult := Group[bool, Student](From(&students).Where(expr.Prop("Age").BInt(0).Gen()), func(student Student) bool {
...
func TestWhereAnySifu(t *testing.T) {
	...
	mat := From(&UserList).Where(expr.Prop("Id").SmallerThanInt(5).Gen()).Where(
	...
	assertion2 := From(&UserList).Where(expr.Prop("Id").SmallerThanInt(5).Gen()).Any(expr.Prop("Username").EqStr("Wade").Gen()).Assert()
...
func TestOpFusionWithSifu(t *testing.T) {
	...
	groupped := Group[bool, Person](
		From(&personList).Where(expr.Prop("Identifier").BInt(0).Gen()), Sifu.KeyAs[Person, bool](expr.Prop("Active")).Gen(),
	).Collect()
...
func TestProject1WithSifu(t *testing.T) {
	...
	newUsers = Project[Person, SysUser](
		From(&personList).Where(expr.Prop("Identifier").BInt(0).Gen()),
		MapPersonToSysUser,
	)
...
func TestGroupComprehensiveWithSifu(t *testing.T) {
	...
	groupedFiltered := Group[string, Employee](
		From(&employees).Where(expr.Prop("Age").BInt(28).Gen()),
		func(e Employee) string { return e.Department },
	).Collect()
...
func TestProjectTakeSkipFilterWithSifu(t *testing.T) {
	...
	var expr *Sifu.SifuExpr[Employee]
...
func TestGroupFilterTakeSkipWithSifu(t *testing.T) {
	...
	expr := Sifu.Expr[Student]()
	GroupResult := Group[bool, Student](From(&students).Skip(2).Take(2).Where(expr.Prop("Age").BInt(0).Gen()), Sifu.KeyAs[Student, bool](expr.Prop("Pressent")).Gen()).Collect()
...
func TestUpdate2WithSifu(t *testing.T) {
	...
	result := From(&CityList).Where(expr.Prop("Active").False().Gen()).Skip(1).Take(1).
		Update(expr.Prop("Name").AppendString(" Deactivated").Gen()).Collect()
